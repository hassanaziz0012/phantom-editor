...
def get_intel_hardware_encoder_args() -> dict:
    """
    Detects and returns FFmpeg encoding parameters for Intel QuickSync Video (QSV).
    Tests for h264_qsv first; if unavailable (e.g. on Haswell where oneVPL dropped legacy support),
    falls back to h264_vaapi on the Intel iGPU render node, or software libx264.
    """
    # 1. Try native Intel QSV
    qsv_test_cmd = [
    ...
    try:
        if subprocess.run(qsv_test_cmd, capture_output=True).returncode == 0:
            return {
                "type": "qsv",
                ...
                "vcodec": ["-c:v", "h264_qsv", "-global_quality", "20", "-preset", "veryfast"],
                "desc": "Intel QuickSync (h264_qsv)"
            }
    except Exception:
        pass

    # 2. Try Intel VAAPI (QuickSync ASIC hardware encoder block via VAAPI)
    ...
    if intel_node:
        ...
        try:
            if subprocess.run(vaapi_test_cmd, capture_output=True).returncode == 0:
                return {
                    "type": "vaapi",
                    "hw_args": ["-init_hw_device", f"vaapi=va:{intel_node}", "-filter_hw_device", "va"],
                    "hwaccel_args": ["-hwaccel", "vaapi", "-hwaccel_device", intel_node],
                    "filter_prefix": "",
                    "filter_suffix": ",format=nv12,hwupload",
                    "vcodec": ["-c:v", "h264_vaapi", "-qp", "20"],
                    "desc": f"Intel QuickSync via VAAPI (h264_vaapi on {intel_node})"
                }
        except Exception:
            pass
    ...
    return {
        ...
        "desc": "CPU Software (libx264 veryfast)"
    }
