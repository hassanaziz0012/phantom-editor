...
def fetch_all_video_ids(youtube, uploads_playlist_id: str, cached_ids: Optional[set[str]] = None) -> tuple[list[str], bool]:
    ...
    while True:
        ...
        # Print progress status message after every 500 videos collected
        if len(video_ids) // 500 > (len(video_ids) - len(items)) // 500:
            milestone = (len(video_ids) // 500) * 500
            if milestone > 0:
                print(f"      Fetched {milestone} videos")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch all videos from a YouTube channel and cache them.")
    parser.add_argument(
        "--fresh",
        action="store_true",
        help="Force a fresh fetch from the YouTube API, bypassing and updating the cache."
    )
    parser.add_argument(
        "channel",
        nargs="?",
        default=None,
        help="YouTube channel ID or handle (optional, falls back to YOUTUBE_CHANNEL_ID in .env)"
    )
    args = parser.parse_args()

    if not API_KEY:
        print("Error: YOUTUBE_API_KEY must be set in .env", file=sys.stderr)
        sys.exit(1)

    target_channel = args.channel or CHANNEL_ID
    if not target_channel:
        print("Error: A channel must be specified (either as an argument or in YOUTUBE_CHANNEL_ID in .env)", file=sys.stderr)
        sys.exit(1)

    # Resolve channel input using robust utility
    try:
    ...
    try:
        videos = fetch_channel_videos(API_KEY, resolved_id, fresh=args.fresh)
    except Exception as e:
