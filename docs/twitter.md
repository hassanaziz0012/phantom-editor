...
Automates tweeting by launching Playwright with saved credentials from `auth.json`. Supports attaching an image from the clipboard and cross-posting to Substack Notes and Threads.

* **Usage**: `uv run twitter/post_tweet.py [--image] [--substack] [--threads] "tweet content"`

### [tweet.sh](../twitter/tweet.sh)
A shell script wrapper to easily post a tweet using the Python automation script.

* **Usage**: `./twitter/tweet.sh [--image] [--substack] [--threads] "tweet content"`
